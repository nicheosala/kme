...
from sd_qkd_node.model import Key as ModelKey
from sd_qkd_node.model.errors import KeyNotFound
...
async def generate(
    ...
    logging.getLogger("sd_qkd_node").debug(
...
async def pop_block() -> orm.Block | None:
    ...
    async with lock:
        b: orm.Block = (
            await orm.Block.objects.filter(available_bits__gt=0).filter(timestamp__gt=now() - Config.TTL).first()
        )
...
async def save_ksid(ksid: orm.Ksid) -> None:
    ...
    await orm.Ksid.objects.create(
        src=ksid.src, dst=ksid.dst, kme_src=ksid.kme_src, kme_dst=ksid.kme_dst, qos=ksid.qos, ksid=ksid.ksid
    )
